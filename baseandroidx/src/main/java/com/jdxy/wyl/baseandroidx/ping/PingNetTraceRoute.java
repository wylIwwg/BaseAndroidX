...
public class PingNetTraceRoute {
    ...
    public void startTraceRoute(String host) {
        if (isCTrace && loaded) {
            try {
                ...
                ToolLog.i("PingNetTraceRoute----"+"调用jni c函数执行traceroute过程");
            } catch (UnsatisfiedLinkError e) {
                e.printStackTrace();
                // 如果c调用失败改调JAVA代码
                ToolLog.i("PingNetTraceRoute----"+"调用java模拟traceRoute");
    ...
    private String execPing(PingTask ping) {
        ...
        try {
            ...
            ToolLog.i("PingNetTraceRoute--------pingCmd---"+pingCmd);
    ...
    private void execTrace(TraceTask trace) {
        ...
        try {
            ...
            while (!finish && trace.getHop() < 30) {
                ...
                ToolLog.i("PingNetTraceRoute--------command---"+command);
                ...
                if (m.find()) {
                    ...
                    ToolLog.i("PingNetTraceRoute--------pingIp---"+pingIp);
                    ...
                    if (status.length() == 0) {
                        ...
                        ToolLog.i("PingNetTraceRoute----1----string---"+string);
                    ...
                    if (matchPingIp.find()) {
                        ...
                        if (matcherTime.find()) {
                            ...
                            log.append("\t");
                            String string = log.toString();
                            ToolLog.i("PingNetTraceRoute----2----string---"+string);
                            listener.OnNetTraceUpdated(string+"\n");
                        }
                        finish = true;
                    } else {
                        ...
                        String string = log.toString();
                        ToolLog.i("PingNetTraceRoute----3----string---"+string);
                        listener.OnNetTraceUpdated(string+"\n");
                    }
